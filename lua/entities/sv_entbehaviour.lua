if SERVER then
    ...
    local tbois_npc_attack_distance = CreateConVar("tbois_npc_attack_distance", 80, FCVAR_NONE, "The reach of pen's attack.")

    local tbois_npc_attack_interval = CreateConVar("tbois_npc_attack_interval", 0.2, FCVAR_NONE, "The delay between pen's attacks.")

    local tbois_npc_attack_force = CreateConVar("tbois_npc_attack_force", 800, FCVAR_NONE, "The physical force of pen's attack. Higher values throw things \z farther.")

    local tbois_npc_smash_props = CreateConVar("tbois_npc_smash_props", 1, FCVAR_NONE, "If set to 1, pen will punch through any props placed in their way.")

    local tbois_npc_allow_jump = CreateConVar("tbois_npc_allow_jump", 1, FCVAR_NONE, "If set to 1, pen will be able to jump.")

    local tbois_npc_hiding_scan_interval = CreateConVar("tbois_npc_hiding_scan_interval", 3, FCVAR_NONE, "pen will only seek out hiding places every X seconds. This can be an \z expensive operation, so it is not recommended to lower this too much. \z However, if distant pens are not hiding from you quickly enough, you \z may consider lowering this a small amount.")

    local tbois_npc_hiding_repath_interval =
        CreateConVar("tbois_npc_hiding_repath_interval", 1, FCVAR_NONE,
        "The path to pen's hiding spot will be redetermined every X seconds.")

    local tbois_npc_chase_repath_interval =
        CreateConVar("tbois_npc_chase_repath_interval", 0.1, FCVAR_NONE,
        "The path to and position of pen's target will be redetermined every \z
        X seconds.")

    local tbois_npc_expensive_scan_interval =
        CreateConVar("tbois_npc_expensive_scan_interval", 1, FCVAR_NONE,
        "Slightly expensive operations (distance calculations and entity \z
        searching) will occur every X seconds.")

    local tbois_npc_force_download =
        CreateConVar("tbois_npc_force_download", 1, FCVAR_ARCHIVE,
        "If set to 1, clients will be forced to download pen resources \z
    ...
    if tbois_npc_force_download:GetBool() then
        resource.AddWorkshop(workshopID)
    end

    util.AddNetworkString("tbois_nag")
    util.AddNetworkString("tbois_navgen")
    ...
    local VECTOR_tbois_HEIGHT = Vector(0, 0, 96)
    local function isPointSuitableForHiding(point)
        trace.start = point
        trace.endpos = point + VECTOR_tbois_HEIGHT
    ...
    local function buildHidingSpotCache()
        ...
        print(string.format("tbois_npc: found %d suitable (%d unsuitable) hiding \z
    ...
    local function isValidTarget(ent)
        ...
        -- Ignore dead NPCs, other pens, and dummy NPCs.
    ...
    hook.Add("PlayerSpawnedNPC", "penMissingNavmeshNag", function(ply, ent)
        if not IsValid(ent) then return end
        if ent:GetClass() ~= "tbois_npc" then return end
        if navmesh.GetNavAreaCount() > 0 then return end

        -- Try to explain why pen isn't working.
        net.Start("tbois_nag")
        net.Send(ply)
    end)
    ...
    local function navEndGenerate()
        ...
        if not navmesh.IsGenerating() then
            print("tbois_npc: Navmesh generation completed in " .. timeElapsedStr)
        else
            print("tbois_npc: Navmesh generation aborted after " .. timeElapsedStr)
        end
    ...
    local function navGenerate()
        ...
        if next(seeds, nil) == nil then
            print("tbois_npc: Couldn't find any places to seed nav_generate")
            return false
        end
        ...
        for seed in pairs(seeds) do
            ...
            if not tr.StartSolid and tr.Hit then
                print(string.format("tbois_npc: Adding seed %s at %s", seed, pos))
                navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
            else
                print(string.format("tbois_npc: Couldn't add seed %s at %s", seed,
                    pos))
            end
        end

        -- The least we can do is ensure they don't have to listen to this noise.
        for _, pen in pairs(ents.FindByClass("tbois_npc")) do
            pen:Remove()
        end
        ...
        if navmesh.IsGenerating() then
            ...
            hook.Add("ShutDown", "penNavGen", navEndGenerate)
        else
            print("tbois_npc: nav_generate failed to initialize")
    ...
    concommand.Add("tbois_npc_learn", function(ply, cmd, args)
        if navmesh.IsGenerating() then
            return
        end
        ...
        if game.SinglePlayer() then
            print("tbois_npc: Beginning nav_generate requested by " .. ply:Name())

            -- Disable expensive computations in single-player. pen doesn't use
        ...
        elseif isConsole then
            print("tbois_npc: Beginning nav_generate requested by server console")
        else
            return
        end
    ...
    function ENT:GetNearestTarget()
        ...
        local maxAcquireDist = tbois_npc_acquire_distance:GetInt()
        ...
        for _, ent in pairs(acquirableEntities) do
            ...
            -- if `tbois_npc_spawn_protect' = 1.
            --TODO: Only for the first few seconds?
            if tbois_npc_spawn_protect:GetBool() and ent:IsPlayer()
    ...
    function ENT:AttackNearbyTargets(radius)
        local attackForce = tbois_npc_attack_force:GetInt()
        ...
        for _, ent in pairs(nearEntities) do
            if isValidTarget(ent) then
                ...
                -- Give the player a good whack. pen means business.
            ...
            elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
                if not tbois_npc_smash_props:GetBool() then continue end
    ...
    function ENT:AttemptJumpAtTarget()
        ...
        local maxAttackDistance = tbois_npc_attack_distance:GetInt()
    ...
    function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
        ...
        local scanInterval = tbois_npc_expensive_scan_interval:GetFloat()
        ...
        if IsValid(self.CurrentTarget) then
            ...
            local attackInterval = tbois_npc_attack_interval:GetFloat()
            if currentTime - self.LastAttack > attackInterval then
                local attackDistance = tbois_npc_attack_distance:GetInt()
            ...
            local repathInterval = tbois_npc_chase_repath_interval:GetFloat()
            ...
            if self:IsOnGround() and tbois_npc_allow_jump:GetBool()
            ...
            local hidingScanInterval = tbois_npc_hiding_scan_interval:GetFloat()
            ...
            if self.HidingSpot ~= nil then
                local hidingInterval = tbois_npc_hiding_repath_interval:GetFloat()
