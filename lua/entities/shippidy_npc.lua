...
ENT.JumpSound = Sound("shippidy_npc/jump.mp3")
ENT.JumpHighSound = Sound("shippidy_npc/jump.mp3")
ENT.TauntSounds = {
	Sound("shippidy_npc/attack1.mp3"),
	Sound("shippidy_npc/attack2.mp3"),
}
local chaseMusic = Sound("shippidy_npc/chase.mp3")
...
if SERVER then -- SERVER --
    include("sv_entbehaviour.lua")
     ...
     --[ CHANGE MODEL IMAGE ] --
    local MAT_shippidy = Material("npc_shippidy/shippidy_npc")
    killicon.Add("shippidy_npc", "npc_shippidy/killicon", color_white)
    language.Add("shippidy_npc", "shippidy ")
    ...
    -- [][] IGNORE FROM HERE [][] --
            ...
            local lastPanic = 0 -- The last time we were in music range of a shippidy.


            -- If another shippidy comes in range before this delay is up,
            -- the music will continue where it left off.
            local MUSIC_RESTART_DELAY = 2

            -- Beyond this distance, shippidys do not count to music volume.
            local MUSIC_CUTOFF_DISTANCE = 1650

            -- Max volume is achieved when MUSIC_shippidy_PANIC_COUNT shippidys are this close,
            -- or an equivalent score.
            local MUSIC_PANIC_DISTANCE = 450

             -- That's a lot of shippidy.
            local MUSIC_shippidy_PANIC_COUNT = 8

            local MUSIC_shippidy_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_shippidy_PANIC_COUNT

            --TODO: Why don't these flags show up? Bug? Documentation would be lovely.
            local shippidy_npc_music_volume = CreateConVar("shippidy_npc_music_volume", 1, bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE), "Maximum music volume when being chased by shippidy. (0-1, where 0 is muted)")

            local function updatePanicMusic()
                if #ents.FindByClass("shippidy_npc") == 0 then
                    -- Whoops. No need to run for now.
                    DevPrint(4, "Halting music timer.")
                    timer.Remove("shippidyPanicMusicUpdate")
                ...
                local userVolume = math.Clamp(shippidy_npc_music_volume:GetFloat(), 0, 1)
                ...
                for _, ent in pairs(nearEntities) do
                    if IsValid(ent) and ent:GetClass() == "shippidy_npc" then
                ...
                local musicVolume = math.min(1,
                    totalDistanceScore / MUSIC_shippidy_MAX_DISTANCE_SCORE)
                ...
                if musicVolume > 0 then
                    ...
                    if not LocalPlayer():Alive() then
                        -- Quiet down so we can hear shippidy taunt us.
            ...
            local function startTimer()
                if not timer.Exists("shippidyPanicMusicUpdate") then
                    timer.Create("shippidyPanicMusicUpdate", 0.05, REPEAT_FOREVER,
                        updatePanicMusic)
    ...
    function ENT:DrawTranslucent()
        render.SetMaterial(MAT_shippidy)

        -- Get the normal vector from shippidy to the player's eyes, and then compute
        ...
        -- shippidy should only look 1/3 of the way up to the player so that they
...
--
list.Set("NPC", "shippidy_npc", {
	Name = "shippidy", -- ingame npc name
	Class = "shippidy_npc", 
