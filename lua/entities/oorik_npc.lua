...
ENT.JumpSound = Sound("oorik_npc/jump.mp3")
ENT.JumpHighSound = Sound("oorik_npc/jump.mp3")
ENT.TauntSounds = {
	Sound("oorik_npc/attack1.mp3"),
	Sound("oorik_npc/attack2.mp3"),
	Sound("oorik_npc/attack3.mp3")
}
local chaseMusic = Sound("oorik_npc/chase.mp3")
...
if SERVER then -- SERVER --
    include("sv_entbehaviour.lua")
     ...
     --[ CHANGE MODEL IMAGE ] --
    local MAT_oorik = Material("npc_oorik/oorik_npc")
    killicon.Add("oorik_npc", "npc_oorik/killicon", color_white)
    language.Add("oorik_npc", "oorik ")
    ...
    -- [][] IGNORE FROM HERE [][] --
            ...
            local lastPanic = 0 -- The last time we were in music range of a oorik.


            -- If another oorik comes in range before this delay is up,
            -- the music will continue where it left off.
            local MUSIC_RESTART_DELAY = 2

            -- Beyond this distance, ooriks do not count to music volume.
            local MUSIC_CUTOFF_DISTANCE = 1650

            -- Max volume is achieved when MUSIC_oorik_PANIC_COUNT ooriks are this close,
            -- or an equivalent score.
            local MUSIC_PANIC_DISTANCE = 400

             -- That's a lot of oorik.
            local MUSIC_oorik_PANIC_COUNT = 8

            local MUSIC_oorik_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_oorik_PANIC_COUNT

            --TODO: Why don't these flags show up? Bug? Documentation would be lovely.
            local oorik_npc_music_volume = CreateConVar("oorik_npc_music_volume", 1, bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE), "Maximum music volume when being chased by oorik. (0-1, where 0 is muted)")

            local function updatePanicMusic()
                if #ents.FindByClass("oorik_npc") == 0 then
                    -- Whoops. No need to run for now.
                    DevPrint(4, "Halting music timer.")
                    timer.Remove("oorikPanicMusicUpdate")
                ...
                local userVolume = math.Clamp(oorik_npc_music_volume:GetFloat(), 0, 1)
                ...
                for _, ent in pairs(nearEntities) do
                    if IsValid(ent) and ent:GetClass() == "oorik_npc" then
                ...
                local musicVolume = math.min(1,
                    totalDistanceScore / MUSIC_oorik_MAX_DISTANCE_SCORE)
                ...
                if musicVolume > 0 then
                    ...
                    if not LocalPlayer():Alive() then
                        -- Quiet down so we can hear oorik taunt us.
            ...
            local function startTimer()
                if not timer.Exists("oorikPanicMusicUpdate") then
                    timer.Create("oorikPanicMusicUpdate", 0.05, REPEAT_FOREVER,
                        updatePanicMusic)
    ...
    function ENT:DrawTranslucent()
        render.SetMaterial(MAT_oorik)

        -- Get the normal vector from oorik to the player's eyes, and then compute
        ...
        -- oorik should only look 1/3 of the way up to the player so that they
...
--
list.Set("NPC", "oorik_npc", {
	Name = "Oorik", -- ingame npc name
	Class = "oorik_npc", 
