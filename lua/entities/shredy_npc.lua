...
ENT.JumpSound = Sound("shredy_npc/jump.mp3")
ENT.JumpHighSound = Sound("shredy_npc/jump.mp3")
ENT.TauntSounds = {
	Sound("shredy_npc/attack1.mp3"),
	Sound("shredy_npc/attack2.mp3"),
	Sound("shredy_npc/attack3.mp3")
}
local chaseMusic = Sound("shredy_npc/chase.mp3")
...
if SERVER then -- SERVER --
    include("sv_entbehaviour.lua")
     ...
     --[ CHANGE MODEL IMAGE ] --
    local MAT_shredy = Material("npc_shredy/shredy_npc")
    killicon.Add("shredy_npc", "npc_shredy/killicon", color_white)
    language.Add("shredy_npc", "shredy ")
    ...
    -- [][] IGNORE FROM HERE [][] --
            ...
            local lastPanic = 0 -- The last time we were in music range of a shredy.


            -- If another shredy comes in range before this delay is up,
            -- the music will continue where it left off.
            local MUSIC_RESTART_DELAY = 2

            -- Beyond this distance, shredys do not count to music volume.
            local MUSIC_CUTOFF_DISTANCE = 1650

            -- Max volume is achieved when MUSIC_shredy_PANIC_COUNT shredys are this close,
            -- or an equivalent score.
            local MUSIC_PANIC_DISTANCE = 400

             -- That's a lot of shredy.
            local MUSIC_shredy_PANIC_COUNT = 8

            local MUSIC_shredy_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_shredy_PANIC_COUNT

            --TODO: Why don't these flags show up? Bug? Documentation would be lovely.
            local shredy_npc_music_volume = CreateConVar("shredy_npc_music_volume", 1, bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE), "Maximum music volume when being chased by shredy. (0-1, where 0 is muted)")

            local function updatePanicMusic()
                if #ents.FindByClass("shredy_npc") == 0 then
                    -- Whoops. No need to run for now.
                    DevPrint(4, "Halting music timer.")
                    timer.Remove("shredyPanicMusicUpdate")
                ...
                local userVolume = math.Clamp(shredy_npc_music_volume:GetFloat(), 0, 1)
                ...
                for _, ent in pairs(nearEntities) do
                    if IsValid(ent) and ent:GetClass() == "shredy_npc" then
                ...
                local musicVolume = math.min(1,
                    totalDistanceScore / MUSIC_shredy_MAX_DISTANCE_SCORE)
                ...
                if musicVolume > 0 then
                    ...
                    if not LocalPlayer():Alive() then
                        -- Quiet down so we can hear shredy taunt us.
            ...
            local function startTimer()
                if not timer.Exists("shredyPanicMusicUpdate") then
                    timer.Create("shredyPanicMusicUpdate", 0.05, REPEAT_FOREVER,
                        updatePanicMusic)
    ...
    function ENT:DrawTranslucent()
        render.SetMaterial(MAT_shredy)

        -- Get the normal vector from shredy to the player's eyes, and then compute
        ...
        -- shredy should only look 1/3 of the way up to the player so that they
...
--
list.Set("NPC", "shredy_npc", {
	Name = "Shredy", -- ingame npc name
	Class = "shredy_npc", 
