...
ENT.JumpSound = Sound("epicroman_npc/jump.mp3")
ENT.JumpHighSound = Sound("epicroman_npc/jump.mp3")
ENT.TauntSounds = {
	Sound("epicroman_npc/attack1.mp3"),
	Sound("epicroman_npc/attack2.mp3"),
	Sound("epicroman_npc/attack3.mp3")
}
local chaseMusic = Sound("epicroman_npc/chase.mp3")
...
if SERVER then -- SERVER --
    include("sv_entbehaviour.lua")
     ...
     --[ CHANGE MODEL IMAGE ] --
    local MAT_epicroman = Material("npc_epicroman/epicroman_npc")
    killicon.Add("epicroman_npc", "npc_epicroman/killicon", color_white)
    language.Add("epicroman_npc", "epicroman ")
    ...
    -- [][] IGNORE FROM HERE [][] --
            ...
            local lastPanic = 0 -- The last time we were in music range of a epicroman.


            -- If another epicroman comes in range before this delay is up,
            -- the music will continue where it left off.
            local MUSIC_RESTART_DELAY = 2

            -- Beyond this distance, epicromans do not count to music volume.
            local MUSIC_CUTOFF_DISTANCE = 1650

            -- Max volume is achieved when MUSIC_epicroman_PANIC_COUNT epicromans are this close,
            -- or an equivalent score.
            local MUSIC_PANIC_DISTANCE = 400

             -- That's a lot of epicroman.
            local MUSIC_epicroman_PANIC_COUNT = 8

            local MUSIC_epicroman_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_epicroman_PANIC_COUNT

            --TODO: Why don't these flags show up? Bug? Documentation would be lovely.
            local epicroman_npc_music_volume = CreateConVar("epicroman_npc_music_volume", 1, bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE), "Maximum music volume when being chased by epicroman. (0-1, where 0 is muted)")

            local function updatePanicMusic()
                if #ents.FindByClass("epicroman_npc") == 0 then
                    -- Whoops. No need to run for now.
                    DevPrint(4, "Halting music timer.")
                    timer.Remove("epicromanPanicMusicUpdate")
                ...
                local userVolume = math.Clamp(epicroman_npc_music_volume:GetFloat(), 0, 1)
                ...
                for _, ent in pairs(nearEntities) do
                    if IsValid(ent) and ent:GetClass() == "epicroman_npc" then
                ...
                local musicVolume = math.min(1,
                    totalDistanceScore / MUSIC_epicroman_MAX_DISTANCE_SCORE)
                ...
                if musicVolume > 0 then
                    ...
                    if not LocalPlayer():Alive() then
                        -- Quiet down so we can hear epicroman taunt us.
            ...
            local function startTimer()
                if not timer.Exists("epicromanPanicMusicUpdate") then
                    timer.Create("epicromanPanicMusicUpdate", 0.05, REPEAT_FOREVER,
                        updatePanicMusic)
    ...
    function ENT:DrawTranslucent()
        render.SetMaterial(MAT_epicroman)

        -- Get the normal vector from epicroman to the player's eyes, and then compute
        ...
        -- epicroman should only look 1/3 of the way up to the player so that they
